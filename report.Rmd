...
train_data %>% ggplot(aes(x=heart_disease_mortality_per_100k))+
              ...
              annotate("text", x=hd_mean+45, y=100, 
                       label="<- mean", size=2.5, color = "blue")+
              annotate("text", x=hd_med-45, y=100, 
...
$$
\mbox{P}(\mbox{diabetes and obese}) = \mbox{P}(\mbox{diabetes}) * \mbox{P}(\mbox{obese)}
$$
$$
\mbox{P}(\mbox{diabetes and physical inactivity}) = \mbox{P}(\mbox{diabetes}) * \mbox{P}(\mbox{physical inactivity)}
$$
$$
\mbox{P}(\mbox{obese and physical inactivity}) = \mbox{P}(\mbox{obese}) * \mbox{P}(\mbox{physical inactivity)}
$$
$$
\mbox{P}(\mbox{diabetes and obese and physical inactivity}) = \mbox{P}(\mbox{diabetes}) * \mbox{P}(\mbox{obese)} * \mbox{P}(\mbox{physical inactivity})
$$
...
We will just train Rborist against the train and test sets, since we have seen in all cases that we are liklely introducing overfitting when using the entire dataset to predict. We will use ranges of predFixed from 2 thru 5, and minNodes as even numbers from 2 thru 50. 
...
```{r, echo=FALSE}
results[c(1,5,7,9:12), ] %>% arrange(RMSE) %>% grid.table(rows=NULL)
```
